...
export const persistCheckResult = async (
  ...
  `).bind(
    check.id, run.id, result.state, result.statusCode, result.latencyMs, result.error,
    result.xRuntimeMs ?? null, result.serverTiming ? JSON.stringify(result.serverTiming) : null, result.checkedAt
  ).run();
  ...
  const failureCountRow = await db.prepare(`
    ...
    FROM check_results
    WHERE check_id = ? AND state = 'fail' AND checked_at >= ?
  `).bind(check.id, incidentStartedAt).first<{ count: number }>();
  
  const failureCount = Number(failureCountRow?.count ?? 0);
  const statements = [buildUpdateCheckStatement(db, nextCheck, certificate, result.checkedAt)];

  if (nextCheck.last_state === "fail" && unresolvedIncident) {
  ...
  if (shouldInsertStatusEvent) {
    statements.push(buildStatusEventStatement(db, check, run, result, evaluated.transition));
  }
