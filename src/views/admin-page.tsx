...
const AdminPanel = ({ data, feedback = null }: { data: AdminData; feedback?: string | null }) => (
  <section id="admin-panel" class="shell p-6">
    ...
    <section id="admin-assignment-section" class="mt-8 grid gap-6 lg:grid-cols-2">
      ...
      <div id="admin-checks-panel" class="glass-surface rounded-lg p-4"><h3 id="admin-checks-title" class="font-black text-slate-50">監視対象の group 移動</h3><div id="admin-checks-list" class="mt-4 grid gap-3">
        {data.checks.map((check) => <form id={`admin-check-${check.id}`} class="rounded-md border border-slate-700/70 p-3" method="post" action={`/admin/checks/${check.id}/group`} hx-post={`/admin/checks/${check.id}/group`} hx-target="#admin-panel" hx-swap="outerHTML"><p class="font-bold text-slate-100">{check.name}</p><p class="truncate text-xs text-slate-400">{check.url}</p><div class="mt-2 flex gap-2"><select id={`admin-check-${check.id}-group`} class="glass-input min-w-0 flex-1 rounded-md px-2 py-2 text-sm" name="group_id" required>{data.groups.map((group) => <option value={group.id} selected={group.id === check.group_id}>{group.name} ({group.slug})</option>)}</select><button id={`admin-check-${check.id}-move`} class="glass-button rounded-md px-3 py-2 text-sm font-bold" type="submit">移動</button></div></form>)}
      </div></div>
    </section>
  </section>
);
