...
<html lang="ja">
  ...
  <body class="min-h-screen text-slate-100">
    <header class="sticky top-0 z-50 w-full border-b border-white/15 bg-slate-950/55 backdrop-blur-2xl">
      <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center gap-2">
          <a href="/" class="glass-button inline-flex items-center rounded-xl px-4 py-2 text-sm font-semibold text-slate-100">概要</a>
          <a href="/checks" class="glass-button inline-flex items-center rounded-xl px-4 py-2 text-sm font-semibold text-slate-100">更新</a>
        </div>
      </div>
    </header>
    ${body}
    ...
    <div class="flex flex-col gap-4">
      ...
      <div class="grid gap-3 xl:grid-cols-2">
        <label class="grid gap-1 text-sm">
          ...
          <input name="name" required value="${escapeHtml(check.name)}" class="glass-input rounded-xl px-3 py-2 text-slate-100 placeholder:text-slate-400" />
        </label>
        <label class="grid gap-1 text-sm">
          <span class="font-semibold text-slate-200">URL</span>
          <input name="url" required value="${escapeHtml(check.url)}" class="glass-input rounded-xl px-3 py-2 text-slate-100 placeholder:text-slate-400" />
        </label>
        <label class="grid gap-1 text-sm">
      ...
      <div class="flex flex-wrap gap-2">
        <button id="check-item-${check.id}-save" class="glass-button inline-flex items-center rounded-xl bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-950">保存</button>
        <a
...
const renderCreateForm = (page: number) => `
  <div id="checks-create-form-wrap" hidden>
    <form id="checks-create-form" class="glass-surface mt-4 grid gap-3 rounded-3xl p-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,2fr)_repeat(4,minmax(0,0.8fr))_minmax(0,0.8fr)]" hx-post="/checks?page=${page}" hx-target="#content" hx-swap="outerHTML show:top">
        ...
        <div class="flex items-end gap-2">
          <button id="checks-create-submit" class="glass-button inline-flex h-10 w-full min-w-0 items-center justify-center rounded-xl bg-slate-50 px-4 text-sm font-semibold text-slate-950 xl:self-end">追加</button>
          <button id="checks-create-close" type="button" class="glass-button inline-flex h-10 items-center justify-center rounded-xl px-4 text-sm font-semibold text-slate-100 xl:self-end">閉じる</button>
        </div>
      </form>
  </div>
`;
    ...
    <section id="checks-pagination-panel" class="glass-surface rounded-3xl p-5">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap gap-2">
          ${
            hasPrev
              ? `<a id="checks-pagination-prev" href="/checks?page=${prevPage}" hx-get="/checks?page=${prevPage}" hx-target="#content" hx-swap="outerHTML show:top" class="glass-button inline-flex items-center rounded-xl px-4 py-2 text-sm font-semibold text-slate-100">前へ</a>`
              : `<span id="checks-pagination-prev" class="inline-flex items-center rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-slate-400">前へ</span>`
          }
          <span id="checks-pagination-current" class="inline-flex items-center rounded-xl border border-white/15 bg-white/10 px-4 py-2 text-sm font-semibold text-slate-100">${page} / ${totalPages}</span>
          ${
            hasNext
              ? `<a id="checks-pagination-next" href="/checks?page=${nextPage}" hx-get="/checks?page=${nextPage}" hx-target="#content" hx-swap="outerHTML show:top" class="glass-button inline-flex items-center rounded-xl px-4 py-2 text-sm font-semibold text-slate-100">次へ</a>`
              : `<span id="checks-pagination-next" class="inline-flex items-center rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-slate-400">次へ</span>`
          }
        </div>
      </div>
    </section>
          ...
          ${
            data.checks.length > 0
              ? data.checks
                  ...
                  .join("")
              : '<p id="checks-empty" class="glass-surface rounded-2xl border border-dashed border-white/15 p-4 text-sm text-slate-200">まだ監視対象がありません。</p>'
          }
        </div>
      </section>
