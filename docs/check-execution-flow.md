...
consumer が dequeue
  ↓
check_runs を確認して未完了なら実行
  ↓
check_runs.started_at を更新
  ↓
HTTP fetch
  ↓
...
`runCheck()` の開始時点で `started_at` を入れます。これは「consumer がこの run を処理し始めた時刻」です。
...
- `started_at` は consumer 開始時刻であり、送信済み判定ではない
- `started_at` は `runCheck()` 開始時に更新する
