...
`tls_valid_to` と監視実行時刻から算出した残日数が 30 日以内の場合は、証明書警告として表示します。これは監視対象の `fail` や incident にはしません。証明書期限切れや TLS 接続・検証失敗は `fail` 扱いにします。`tls_days_remaining` は probe 時点の参考値であり、判定には使いません。
